...
def ultralight2(interface, userdata, message):
    ...
    if len(values) != len(keys):
        logger.warning(
            "Syntax error in ultralight2 payload {} (odd number of sections)".format(
                payload
            )
        )
...
class MqttInterface(Interface):
    def __init__(self, handler, config):
        ...
        if self.parserName == "ultralight2":
            ...
            self.parser = None
            logger.info(
                f"MQTT Interface {self.name} has no parser given. Ignoring incoming messages..."
            )
        ...
        if self.user:
            ...
            if not self.tls and not self.tls is False:
                self.client.tls_set(
                    ca_certs=None, certfile=None, keyfile=None, ciphers=None
                )

        self.client.on_message = lambda client, userdata, message: self.on_message(
    ...
    def on_message(self, client, userdata, message):
        ...
        payload = message.payload
        logger.debug(
            "Received MQTT message with topic {} and payload {}.".format(topic, payload)
        )
        if self.parser:
    ...
    def on_connect(self, client, userdata, flags, rc):
        if rc == 0:
            ...
            logger.error(
                "MQTT interface {} connection failed: unacceptable protocol version".format(
                    self.name
                )
            )
        elif rc == 2:
            logger.error(
                "MQTT interface {} connection failed: identifier rejected".format(
                    self.name
                )
            )
        elif rc == 3:
            logger.error(
                "MQTT interface {} connection failed: servr unavailable".format(
                    self.name
                )
            )
        elif rc == 4:
            logger.error(
                "MQTT interface {} connection failed: bad user name or password".format(
                    self.name
                )
            )
        elif rc == 5:
            logger.error(
                "MQTT interface {} connection failed: not authorized".format(self.name)
            )
        else:
            logger.error(
                "MQTT interface {} connection failed with return code {}".format(
                    self.name, rc
                )
            )
    ...
    def start(self):
        ...
        if not self.client.is_connected():
            logger.warning(
                f"MQTT interface {self.name} did not connect successfully on first try."
            )
    ...
    def registerSubscriber(self, id, node):
        if id in self.subs:
            logger.error(
                "The MQTT ID {} already exists for the interface {}".format(
                    id, self.name
                )
            )
            return False
    ...
    def setValue(self, id, value, dryrun=False):
        ...
        if dryrun:
            logger.debug(
                "Dryrun on interface {} with message ID {} successfull.".format(
                    self.name, id
                )
            )
            return True
    ...
    def publish(self, payload, topicPub=None):
        ...
        if not self.client.is_connected():
            logger.error(
                f"Could not Publish via Interface {self.name} to topic {topicPub} with payload {payload} due to not being connected!"
            )
            return False
        if self.dryrun:
            logger.debug(
                "[Dryrun] Publishing via Interface {} to topic {} with payload {}...".format(
                    self.name, topicPub, payload
                )
            )
            return True
        logger.debug(
            "Publishing via Interface {} to topic {} with payload {}...".format(
                self.name, topicPub, payload
            )
        )
