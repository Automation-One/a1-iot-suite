...
def main():
    ...
    parser.add_argument(
        ...
        "-M",
        type=str,
        help="Specifies the method. Can be rtu (for Serial connections) or tcp (default: rtu)",
    )

    # TCP
    parser.add_argument("--host", "-H", type=str, help="Modbus Host (for tcp)")
    parser.add_argument(
        "--port", "-p", type=int, help="Port for the tcp connection (default: 503)"
    )
    ...
    parser.add_argument(
        "--device",
        "-D",
        help="Serial device for sending the request. (Default: /dev/ttymxc2)",
    )
    parser.add_argument(
        "--baudrate", "-b", type=int, help="Baudrate for the serial connection."
    )
    ...
    parser.add_argument(
        ...
        "-S",
        type=int,
        help="Stopbits for the serial connection. (default = 1)",
    )
    parser.add_argument(
        "--bytesize",
        "-B",
        type=int,
        help="Bytesize for the serial connection. (default = 8)",
    )
    ...
    parser.add_argument(
        "--address", "-a", type=int, help="Modbus Address (default = 0)"
    )
    parser.add_argument(
        "--functionCode",
        "-f",
        type=int,
        help="Functioncode for the modbus request (default = 3, supported: 1,3,4,6,15,16)",
    )
    parser.add_argument(
        "--count", "-c", type=int, help="Count for the Modbus Request (default = 1)"
    )

    parser.add_argument(
        "values", nargs="*", type=str, help="Value(s) for functioncode 6"
    )
    ...
    parser.add_argument(
        ...
        dest="verbose",
        action="store_true",
        help="Verbose Output Including the returned register values",
    )
    parser.add_argument(
        "--version",
        dest="version",
    ...
    parser.add_argument(
        ...
        type=str,
        default="int16",
        help="Data format for the read values: int, uint, float with 16, 32 or 64 bit (default = int16)",
    )
    ...
    if args.method == "rtu":
        if not args.baudrate:
            logger.error(
                "The Baudrate must be specified either in the config file or using --baudrate for rtu connections"
            )
            return False

        if not args.parity:
            logger.error(
                "Pairity must be specified either in the config file or  using --parity for rtu connections"
            )
            return False
        ...
        if not args.host:
            logger.error(
                "The Host must be specified either in the config file or using --host for tcp connections"
            )
            return False
    ...
    if args.functionCode in [6, 15] and args.count != 1:
        logger.warning(
            "For Function Codes 6 and 15, the count is induced by the number of Arguments given. => Ignoring Count"
        )
        args.count = 1
...
def run(args):
    ...
    if args.functionCode in [6, 15, 16]:
        builder = BinaryPayloadBuilder(
            byteorder=args.byteorder, wordorder=args.wordorder
        )
    ...
    if args.functionCode == 1:
        ...
        if args.count > 1:
            logger.error(
                "FunctionCode 6 only supports writing to one (16 bit) register!"
            )
            return False
    ...
    if args.functionCode in [3, 4]:
        decoder = BinaryPayloadDecoder.fromRegisters(
            result.registers, byteorder=args.byteorder, wordorder=args.wordorder
        )
        values = []
